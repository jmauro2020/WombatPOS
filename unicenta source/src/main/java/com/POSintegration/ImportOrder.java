  ...
  extends SvrProcess
{
  ...
  private String createAllocation(MOrder order)
  {
    ...
    if (order.getPaymentRule().compareTo("P") == 0)
    {
      ...
      this.log.warning("Verifique que ya existe el Numero de factura" + order.getC_Invoice_ID());
      count = 0;
      while ((!complete) && (count < 5))
      {
        MInvoice tempinv = new MInvoice(getCtx(), order.getC_Invoice_ID(), get_TrxName());
        if ((tempinv != null) && 
          (tempinv.getDocStatus().compareTo("CO") == 0)) {
          complete = true;
        }
      ...
      querya = new Query(Env.getCtx(), "C_AllocationLine", "C_AllocationHdr_ID = ? ", 
        get_TrxName())
        .setParameters(new Object[] {Integer.valueOf(Allocation.getC_AllocationHdr_ID()) })
        .list();
      ...
      {
        this.log.severe("Allocation Process Failed: " + order + " - " + Allocation.getProcessMsg());
      ...
      MBankAccount ba = 
        ...
        (MBankAccount)new Query(getCtx(), "C_BankAccount", whereClause, get_TrxName()).setParameters(new Object[] {Integer.valueOf(order.getAD_Org_ID()), Integer.valueOf(order.getC_Currency_ID()) }).setOrderBy("IsDefault DESC").first();
      ...
      else if (Allocation.getDocStatus() == "CO")
      {
